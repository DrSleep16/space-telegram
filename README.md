...
2. В файле `.env` определите переменную `API_KEY` и укажите в ней ваш API-ключ, например:
   ```
   API_KEY=YOUR_API_KEY_HERE
   ```
3. Аналогично определите переменные `CHANNEL_ID` (id вашего канала в телеграмме),`INTERVAL_HOURS` (частота публикаций в канале), `TOKEN` (токен вашего бота).
