...
1. Убедитесь, что у вас настроены и работают скрипты `spacebot.py`, `epic.py`, `fetch_spacex_images.py` и `apod.py`. Если какой-то из этих скриптов не настроен, следуйте инструкциям, предоставленным для каждого из них.
...
3. Дождитесь завершения выполнения скрипта. После этого все необходимые изображения будут скачаны и сохранены в соответствующих папках.
...
2. Дождитесь завершения выполнения скрипта. После этого в папке `images` появятся скачанные фотографии.
...
2. Запустите скрипт:
   ...
   - Выполните следующую команду:
     ```
     python epic.py --download_count 5
     ```
   - Скрипт начнет скачивать 5 (либо указанное вами число) изображений с использованием вашего API-ключа от NASA и сохранять их в папке `images`.

3. Дождитесь завершения выполнения скрипта. После этого в папке `images` появятся скачанные изображения.
...
2. Запустите скрипт:
   ...
   - Выполните следующую команду:
     ```
     python apod.py  --download_count 5
     ```
   - Скрипт начнет скачивать 5 (или другое указанное вами число) изображений с использованием вашего API-ключа от NASA и сохранять их в папке `images`.
